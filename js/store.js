...
// Helper to get all cases as array for lists
...
export const movePromotionToCase = (promoId, caseId) => {
    // 1. Find Promo
    const promoIdx = appData.promotions.findIndex(p => p.id === promoId);
    if (promoIdx === -1) return false;
    const promo = appData.promotions[promoIdx];

    // 2. Find Case
    const targetCase = appData.cases[caseId];
    if (!targetCase) return false;

    // 3. Convert Promo to Case Image
    const newImage = {
        id: promo.id,
        url: promo.url,
        name: promo.name || 'Anexo',
        type: 'Promoción',
        summary: promo.aiAnalysis?.summary || 'Sin resumen',
        date: new Date().toISOString().split('T')[0], // Use simplified date
        deadline: null,
        nextAction: null
    };

    // 4. Add to Case
    targetCase.images = targetCase.images || [];
    targetCase.images.unshift(newImage);

    // 5. Remove from Promotions
    appData.promotions.splice(promoIdx, 1);
